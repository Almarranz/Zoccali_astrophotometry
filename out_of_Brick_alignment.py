...
for chip in range(chip,chip+1):
    ...
    if band=='H' and chip==3:
        ...
        if more==0: #only 1 reference star
            ...
            xm_ref,ym_ref= 1440 ,630# xm_ref is GNS
            xm_ref=xm_ref*0.5
            ym_ref=ym_ref*0.5
            xm,    ym    =1374,1492
            xoff = xm_ref - xm
        ...
        if more==1: # 2 reference stars
            xm_ref1,ym_ref1=  1440 ,630# xm_ref is GNS
            xm_ref1=xm_ref1*0.5
            ym_ref1=ym_ref1*0.5
            xm1,    ym1    =1374,1492
            xoff1 = xm_ref1 - xm1
            ...
            xm_ref2,ym_ref2=1655.29 ,1318.36# xm_ref is GNS
            xm_ref2=xm_ref2*0.5
            ym_ref2=ym_ref2*0.5
            xm2,    ym2    =1487.4597168,1834.5284424
        ...
        if more==1:
            ...
            xm_ref1,ym_ref1=1531 ,1421 # xm_ref is GNS
            xm1,ym1        = 1426,1887
            xm_ref1=xm_ref1*0.5
            ym_ref1=ym_ref1*0.5
            xoff1 = xm_ref1 - xm1
            ...
            xm_ref2,ym_ref2=   11859  ,1444# xm_ref is GNS
            xm_ref2=xm_ref2*0.5
            ym_ref2=ym_ref2*0.5
            xm2,    ym2    =1590, 1896
...
displa=np.array([x_dis,y_dis]).T
zoc_txt=np.c_[zoc_txt,displa]
np.savetxt(tmp+'GNS_commons_w_Zoc_c%s_out_Brick.txt'%(chip),gns_txt,header='x_gns, dx_gns, y_gns, dy_gns, raH, draH, decH, ddecH, mJ, dmJ, mH, dmH, mK, dmK, H-Ks')
np.savetxt(tmp+'Zoc_c%s_commons_w_GNS_out_Brick.txt'%(chip),zoc_txt,header='a ,d , m, dm, f, df,x,y,dx,dy,x_dis,y_dis. X and Y are the correspondig coorinates wit GNS, They are not the original ones!!!!')
np.savetxt(tmp+'dis_xy_chip%s_out_Brick.txt'%(chip),displa,header='Displacement in pixels.')
...
GNS=np.loadtxt(tmp+'GNS_commons_w_Zoc_c%s_out_brick.txt'%(chip))
...
stars=np.loadtxt(tmp+'Zoc_c%s_commons_w_GNS_out_Brick.txt'%(chip))
stars=stars[valid]
...
np.savetxt('/Users/amartinez/Desktop/PhD/python/Gaussian_fit/'+'arcsec_vx_vy_chip%s_out_Brick.txt'%(chip),rap,header='vx,vy,dvx,dvy (in arcsec/yr)')
...
np.savetxt('/Users/amartinez/Desktop/PhD/python/Gaussian_fit/'+'vx_vy_chip%s_out_Brick.txt'%(chip),ls_tosave,header='vx,vy,dvx,dvy (in km/s)')
